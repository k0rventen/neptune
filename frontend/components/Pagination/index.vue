<template>
    <div class="flex items-center text-black">
        ...
        <button v-for="index in nbDisplayedPages()" :key="index" class="px-3 py-1" :class="{'bg-[#a6d1e6] rounded-md text-white' : index === currentPage - offset }" @click="setPage(index)">
            {{ index + offset }}
        </button>
        <button v-if="offset + 5 !== nbPages && nbPages >= 5" :key="index" class="px-3 py-1">
            ...
        </button>
