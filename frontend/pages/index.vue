      ...
      <div class="h-1/3 px-3 py-3">
        <form @submit.prevent="sendRegisteriesData()">
          <label class="mb-2 gap-3">
            URL :
            <input v-model="registry.url" type="text" class="rounded-md border border-gray-400 outline-none px-2 py-1">
          </label>

          <label for="" class="my-2 gap-3 block">
            Nom d'utilisateur : 
            <input v-model="registry.user" type="text" class="rounded-md border border-gray-400 outline-none px-2 py-1">
          </label>

          <label for="" class="mb-2 gap-3 block">
            Mot de passe
            <input v-model="registry.password" type="password" class="rounded-md border border-gray-400 outline-none px-2 py-1">
          </label>
          <button type="submit" class="px-3 py-1
           rounded-md bg-neptune-blue text-white">Envoyer</button>
        </form>
      </div>
    </Modal>
    <div v-if="this.stats" class="grid grid-cols-4 gap-9">
      <div class="bg-white px-3 py-3 shadow-md rounded-xl flex justify-between text-secondary col-span-4 lg:col-span-1">
        <div>
          <p>Nombre total de vulnérabilités</p>
          <p class="font-bold">{{ this.stats.vulnerabilities_total_count }}</p>
        </div>
        <div class="bg-neptune-blue px-3 py-3 items-center justify-center rounded-xl max-h-fit">
      ...
      <div class="bg-white px-3 py-3 shadow-md rounded-xl flex justify-between text-secondary col-span-4 lg:col-span-2">
        <div>
      ...
      <div class="bg-white px-3 py-3 h-96 shadow-md rounded-xl col-span-4 lg:col-span-2 text-secondary">
      ...
      <div class="bg-white px-3 py-3 h-96 shadow-md rounded-xl text-white col-span-4 md:col-span-2 lg:col-span-1">
        <div class="w-full h-full bg-neptune-blue rounded-xl px-5 py-5 relative overflow-hidden">
          ...
          <p>Neptune est un outil permettant d'avoir un visuel sur les images de son parc.</p>
      ...
      <div class="bg-white px-3 py-3 h-96 shadow-md rounded-xl text-white flex justify-center items-center col-span-4 md:col-span-2 lg:col-span-1">
        <apexchart height="90%" type="radialBar" :options="vulnerabitily.chartOptions" :series="vulnerabitily.series"/>
      </div>
      <div class="bg-white px-3 py-3 h-96 shadow-md rounded-xl col-span-4 lg:col-span-2 text-white">
      ...
      <div class="bg-white px-3 py-3 shadow-md rounded-xl col-span-4">
          ...
          <div class="text-white grid grid-cols-4 gap-9">
            <div v-for="image,index in fiveImg" :key="image.image_id" class="px-5 rounded-lg py-5 relative overflow-hidden cursor-pointer col-span-4 lg:col-span-1" :class="setBgColor(index)" @click="$router.push({ path: '/images/' + image.sha } )">
              <svg v-if="(index === 0)" class="absolute opacity-20 right-0 -bottom-2" enable-background="new 0 0 274.989 274.989" version="1.1" viewBox="0 0 274.99 274.99" width="128" height="128" fill="white">
                <path d="m179.57 216.04c-5.609 11.237-8.425 16.844-14.02 19.641-5.609 5.603-16.837 5.603-28.068 5.603-14.02 0-22.432 0-28.038-5.603-5.624-2.797-8.432-8.404-14.03-19.641 0 0-2.816-2.781-2.816-5.611l-11.228 2.83 8.429 28.025 16.827 14.049c5.598 14.022 16.835 19.652 30.855 19.652 5.609 0 14.039-2.836 16.835-5.63 5.623-2.816 11.232-8.415 14.04-14.022l16.826-14.049 8.425-28.025-11.222-2.83c-2.815 2.83-2.815 5.611-2.815 5.611zm30.891-190.78c-19.664-16.818-42.103-25.266-72.978-25.266-30.855 0-53.295 8.448-72.948 25.266-16.831 14.021-28.039 36.463-28.039 61.743 0 22.422 2.787 39.249 8.4 47.7 8.426 14.008 14.04 22.428 14.04 28.032l2.797 22.43 39.28 19.663 8.432 16.84c2.782 0 5.607 0 11.222 2.797h16.817c14.039 0 22.459-2.797 30.875-2.797l5.603-16.84 39.291-19.663 2.796-22.43c2.818-5.604 2.818-8.426 2.818-11.216 14.018-19.648 19.633-42.094 19.633-64.517-1e-3 -25.279-8.411-47.721-28.039-61.742zm-115.05 137.48c-16.845 0-22.44-11.216-22.44-28.032 0-8.451 0-16.867 5.595-19.652 2.802-5.626 8.434-8.415 16.845-8.415 16.812 0 25.252 8.415 25.252 25.237 0 19.646-8.44 30.862-25.252 30.862zm50.509 30.855c-2.831 0-5.633 0-8.44-2.806-2.817 2.806-2.817 2.806-5.595 2.806-2.836 0-5.609 0-8.425-2.806-2.797 0-2.797-2.791-2.797-5.618 0-2.776 2.797-8.387 5.613-11.198 5.609-5.621 8.387-11.232 11.204-16.837 2.807 5.604 8.44 11.216 11.231 16.837 5.604 2.811 5.604 8.422 5.604 11.198 0 5.618-2.797 8.424-8.395 8.424zm36.464-30.855c-16.837 0-25.263-11.216-25.263-28.032 0-16.867 8.426-28.068 25.263-28.068 8.409 0 14.018 2.79 16.826 8.415 5.613 2.785 5.613 8.411 5.613 16.821-1e-3 19.648-8.421 30.864-22.439 30.864z" fill="white"/>
              </svg>
              <svg v-if="(index === 1)" class="absolute opacity-20 right-0 -bottom-2"  width="128" height="128" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill-rule="evenodd" clip-rule="evenodd" fill="white"><path d="M21.167 21h1.833v2h-22v-2h1.833l7.334-20h3.666l7.334 20zm-4.527-7h-9.274l-1.005 3h11.328l-1.049-3zm-2.919-8h-3.471l-1.005 3h5.525l-1.049-3z"/></svg>
              <svg v-if="(index === 2)" class="absolute opacity-20 right-0 -bottom-3"  width="128" height="128" viewBox="0 0 24 24" clip-rule="evenodd" fill="white" stroke-linejoin="round" stroke-miterlimit="2" xmlns="http://www.w3.org/2000/svg"><path d="m2.095 19.886 9.248-16.5c.133-.237.384-.384.657-.384.272 0 .524.147.656.384l9.248 16.5c.064.115.096.241.096.367 0 .385-.309.749-.752.749h-18.496c-.44 0-.752-.36-.752-.749 0-.126.031-.252.095-.367zm1.935-.384h15.939l-7.97-14.219zm7.972-6.497c-.414 0-.75.336-.75.75v3.5c0 .414.336.75.75.75s.75-.336.75-.75v-3.5c0-.414-.336-.75-.75-.75zm-.002-3c.552 0 1 .448 1 1s-.448 1-1 1-1-.448-1-1 .448-1 1-1z" fill-rule="nonzero"/></svg>
              <svg v-if="(index === 3)" class="absolute opacity-20 right-0 -bottom-8" width="128" height="128" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="white"><path d="M11.5 23l-8.5-4.535v-3.953l5.4 3.122 3.1-3.406v8.772zm1-.001v-8.806l3.162 3.343 5.338-2.958v3.887l-8.5 4.534zm-10.339-10.125l-2.161-1.244 3-3.302-3-2.823 8.718-4.505 3.215 2.385 3.325-2.385 8.742 4.561-2.995 2.771 2.995 3.443-2.242 1.241v-.001l-5.903 3.27-3.348-3.541 7.416-3.962-7.922-4.372-7.923 4.372 7.422 3.937v.024l-3.297 3.622-5.203-3.008-.16-.092-.679-.393v.002z"/></svg>
...
export default {
  ...
  methods: {
    ...mapActions('statistics', ['getStats', 'getHistoricalStats','getFiveImg']),
