...
const columns = [
    {
        name: t('vulnerabilities.active'),
        key: 'ack'
    },
    {
        name: t('vulnerabilities.cve'),
        key: 'name'
    },
    {
        name: t('vulnerabilities.severity'),
        key: 'severity'
    },
    {
        name: t('vulnerabilities.source'),
        key: 'affected_package'
    },
    {
        name: t('vulnerabilities.affected_tags'),
        key: 'affected_images'
    },
    {
        name: t('vulnerabilities.notes'),
        key: 'notes'
    }
]
...
await vulnStore.getVulnerabilities({ ...pagination, filter }).then(() => {
    vulnStore.vulnerabilities.items.forEach((vuln) => {
        copyNotes.value[vuln.name] = vuln.notes
        console.log(copyNotes.value)
    })
})
...
const delaySearch = (input) => {
    ...
    delay.value = setTimeout(async () => {
        ...
        await vulnStore.getVulnerabilities({ ...pagination, filter })
        pagination.page = 1
        refresh.value++
    }, 750)
}
...
const sendNewNotes = async (item) => {
    await vulnStore.sendNotes(item, copyNotes.value[item.name])
}

const changeStatus = async (item) => {
    await vulnStore.sendState(item, !item.active)
}
...
<template>
    <div class="relative bg-white rounded shadow  px-2">
        <div class="flex items-center">
            <input class="w-full py-2 outline-none" type="text" :placeholder="$t('vulnerabilities.vuln_name')"
            @input="delaySearch">
    ...
    2103 609 537 1352 856 2169 930 151 14 608 6 755 -13z" />
        ...
        <div v-if="openParam">
            <label>{{ $t('vulnerabilities.severity_filter') }}:</label>
            <select v-model="filter.severity_filter" class="outline-none">
                <option :value="undefined">{{ $t('vulnerabilities.no_filter') }}</option>
                <option value="unknown">{{ $t('vulnerabilities.unknown') }}</option>
                <option value="negligible">{{ $t('vulnerabilities.negligible') }}</option>
    ...
    <div class="bg-white p-5 mt-5 rounded shadow">
        <custom-table :columns="columns" :data="vulnStore.vulnerabilities.items">
            <template #ack="{ item }">
            ...
            <template #affected_images="{ item }">
                <VDropdown class="w-fit">
                    <button class="bg-primary px-2 py-1 w-16 text-center text-white rounded">
                        {{ item.affected_images.length }}
                    </button>
                    <template #popper>
                        <div class="px-2 py-1">
        ...
        <div class="w-full flex justify-center">
            <paginate class="mt-3" v-model="pagination.page"
                :nbPages="Math.ceil(vulnStore.vulnerabilities.total / vulnStore.vulnerabilities.per_page)" :key="refresh">
            </paginate>
